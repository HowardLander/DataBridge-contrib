...
def main(argv):
   ...
   if (len(sys.argv) < 4):
      ...
      # case we possible parameters are nClusters and nNeighbors
      paramList = sys.argv[3].split("|")
      for thisParam in paramList:
         # first and only parameter should be damping
         paramSplit = thisParam.split(":")
         if (paramSplit[0] == "nClusters"):
             nClusters = int(paramSplit[1])
         if (paramSplit[0] == "nNeighbors"):
             nNeighbors = int(paramSplit[1])

   print 'Input file is "', inputFile
   print 'Output file is "', outputFile
   print 'nClusters is:', str(nClusters)
   print 'nNeighbors is:', str(nNeighbors)
   ...
   #db = SpectralClustering(affinity='precomputed',n_clusters=3,n_neighbors=5)
   db = SpectralClustering(affinity='precomputed', n_clusters=nClusters, n_neighbors=nNeighbors)
