...
def main(argv):
   ...
   if (len(sys.argv) < 4):
      ...
      # case we are only expecting one: nClusters
   ...
   #db = AgglomerativeClustering(affinity='precomputed',linkage='average',connectivity=simMatrix, n_clusters=nClusters)
   db = AgglomerativeClustering(affinity='euclidean', linkage='average', n_clusters=nClusters)
